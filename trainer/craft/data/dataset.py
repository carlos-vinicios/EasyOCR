...
class CraftBaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        qtd_training_files = len(os.listdir(os.path.join(self.data_dir, "ch4_training_images")))
        if self.saved_gt_dir is None or not os.path.exists(self.saved_gt_dir) or len(os.listdir(self.saved_gt_dir)) / 3 < qtd_training_files:
            (
                image,
                region_score,
                affinity_score,
...
class CustomDataset(CraftBaseDataset):
    ...
    def make_gt_score(self, index):
        ...
        and save them to be loaded later with load_saved_gt_score
        ...
        # Save the generated scores to be loaded by load_saved_gt_score
        img_name = self.img_names[index]
        query_idx = img_name.split(".")[0]
        
        # Save region score
        if self.saved_gt_dir is not None and os.path.exists(self.saved_gt_dir):
            # Make sure the saved_gt_dir exists
            os.makedirs(self.saved_gt_dir, exist_ok=True)
            
            # Save region score
            region_score_path = os.path.join(self.saved_gt_dir, f"res_img_{query_idx}_region.jpg")
            cv2.imwrite(region_score_path, (region_score * 255).astype(np.uint8))
            
            # Save affinity score
            affinity_score_path = os.path.join(self.saved_gt_dir, f"res_img_{query_idx}_affi.jpg")
            cv2.imwrite(affinity_score_path, (affinity_score * 255).astype(np.uint8))
            
            # Save confidence mask with threshold 0.6 (as indicated in the load function)
            confidence_mask_path = os.path.join(self.saved_gt_dir, f"res_img_{query_idx}_cf_mask_thresh_0.6.jpg")
            cv2.imwrite(confidence_mask_path, (confidence_mask * 255).astype(np.uint8))
    ...
    def load_saved_gt_score(self, index):
        ...
        query_idx = self.img_names[index].split(".")[0]
